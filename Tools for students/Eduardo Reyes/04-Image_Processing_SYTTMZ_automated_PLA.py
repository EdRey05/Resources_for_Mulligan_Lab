...
Full name of script: Whole image processing for automated PLA experiments [Version 01]
...
Description: This script processess the output images of the EVOS-M7000 when doing Proximity Ligation Assays (PLA) on Ibidi uSlides 0.4 and imaged with the
             automation tool of the imager. This script was used in ~50 experimental conditions (one channel or folder each with 4-8 thousand images) and 
             does 3 main steps, 1)Merges the output images back to hyperstacks, 2)Stitches a user-defined number of images together, 3)Makes a Z-projection
             of the stitched images and subtracts background noise. 
             
             For example, more than half of the folders processed with this script had 4,284 raw images, which were merged into 357 fields of view (3 colours
             with 4 slices each), then they were renamed for easier stitching using the Grid stitching plug-in of ImageJ, then 9 images were stitched with 
             7x5 fields of view + 1 image 7x6, and finally the images were Z-projected and the background noise was subtracted.
             
             NOTE: Even though several parts of this script would work for any number of initial images or stitch different amounts together, there are a few
             crucial steps that work only if the imaging was done at 40x, 3 colours, 4 slices, 15% of total area (357 FOVs per channel/condition), and thus
             would required manual editing before running the script. For example, if there are more or less than 4 slices, more or less than 357 FOVs, or if
             the imaging wasnt set up as serpentine vertical with more overlap in the EVOS software. These can be easily adjusted if needed as well as the
             number of images to stitch together (plus saving names).
...
Version History:
V01 (July 14, 2022): First version where all the individual parts that were written separately are fully integrated and working. The code is fully annotated
                     but there are still some sections that need manual edits. The version used to make the hyperstacks (Part 1 - Merge images) corresponds
                     to the script Images_to_Hyperstacks_merger (can be found in the Github folder "Tools for EVOS-M7000 images" with few adjustments to
                     integrate it with the others (same logic and steps). All the other parts were not consolidated into standalone scripts, just here as a
                     whole. 
                     Pending: 1)Enclose each part and check dependencies to make them fully independent so the user can decide to do all at once or try some
                     parts that may fail with the standard parameters (like the stitching which may require parameter adjustments), 2)Integrate an option to
                     know when the script is done in case a separate PC is used to run the script. 3)Make the option to iterate through multiple condition/
                     experiment folders automatically. 4)A checkbox for optional deletion of the folders containing merged images and stitched images at the
                     end of the script (in case the user only needs to keep the final, processed images.
...
new_names = [ 0, 7, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77, 84, 91, 98,  105, 112, 119, 126, 133, 140, 147, 154, 161, 168, 175,  182, 189, 196, 203, 210,
             217, 224, 231, 238, 245, 252, 259, 266, 273, 280, 287, 294, 301, 308, 315, 322, 329, 336, 343, 350, 351, 344, 337, 330, 323, 316, 309, 302,
             295, 288, 281, 274, 267, 260, 253, 246, 239, 232, 225, 218, 211, 204, 197, 190, 183, 176, 169, 162, 155, 148, 141, 134, 127, 120, 113, 106,
             99, 92, 85, 78, 71, 64, 57, 50, 43, 36, 29, 22, 15, 8, 1, 2, 9, 16, 23, 30, 37, 44, 51, 58, 65, 72, 79, 86, 93, 100, 107, 114, 121, 128,
             135, 142, 149, 156, 163, 170, 177, 184, 191, 198, 205, 212, 219, 226, 233, 240, 247, 254, 261, 268, 275, 282, 289, 296, 303, 310, 317, 324,
             331, 338, 345, 352, 353, 346, 339, 332, 325, 318, 311, 304, 297, 290, 283, 276, 269, 262, 255, 248, 241, 234, 227, 220, 213, 206, 199, 192,
             185, 178, 171, 164, 157, 150, 143, 136, 129, 122, 115, 108, 101, 94, 87, 80, 73, 66, 59, 52, 45, 38, 31, 24, 17, 10, 3, 4, 11, 18, 25, 32,
             39, 46, 53, 60, 67, 74, 81, 88, 95, 102, 109, 116, 123, 130, 137, 144, 151, 158, 165, 172, 179, 186, 193, 200, 207, 214, 221, 228, 235, 242,
             249, 256, 263, 270, 277, 284, 291, 298, 305, 312, 319, 326, 333, 340, 347, 354, 355, 348, 341, 334, 327, 320, 313, 306, 299, 292, 285, 278,
             271, 264, 257, 250, 243, 236, 229, 222, 215, 208, 201, 194, 187, 180, 173, 166, 159, 152, 145, 138, 131, 124, 117, 110, 103, 96, 89, 82, 75,
             68, 61, 54, 47, 40, 33, 26, 19, 12, 5, 6, 13, 20, 27, 34, 41, 48, 55, 62, 69, 76, 83, 90,	97, 104, 111, 118, 125, 132, 139, 146, 153, 160,
             167, 174, 181, 188, 195, 202, 209, 216, 223, 230, 237, 244, 251, 258, 265, 272, 279, 286, 293, 300, 307, 314, 321, 328, 335, 342, 349, 356]
...
stitched_names = ["Row_01_05", "Row_06_10", "Row_11_15", "Row_16_20", "Row_21_25", "Row_26_30", "Row_31_35", "Row_36_40", "Row_41_45", "Row_46_51"]
